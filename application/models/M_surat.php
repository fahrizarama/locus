...
class M_surat extends CI_Model
{
    ...
    {
        $config['upload_path']          = './assets/file_surat/surat_masuk/';
    ...
    public function save_suratk()
    {
        ...
        $this->tembusan_surat = $post['tembusan_surat'];
        $this->penulis_surat = $post['penulis_surat'];
        $this->file_surat = $this->UploadFile2();

        $this->db->insert($this->tabel2, $this);
    }

    private function UploadFile2()
    {
        $config['upload_path']          = './assets/file_surat/surat_keluar/';
        $config['allowed_types']        = 'pdf|docx';
        $nama_lengkap = $_FILES['file_surat']['name'];
        $config['file_name']            = $nama_lengkap;
        $config['overwrite']            = true;
    ...
    public function update_suratk()
    {
        ...
        $this->db->update($this->tabel2, $this, array('id_surat' => $post['id_surat']));
    }
