...
?>
<?php $title = "<i class='far fa-handshake'></i>&nbsp;Transaksi Pemasukan Sebagai Piutang"; ?>
...
<div class="row">
    <div class="col-md-12">
        ...
        <div class="container">
            <form class="form-horizontal" role="form" id="formTransaksi" action="<?= base_url('C_transaksi/add_transaksi_pemasukan_piutang') ?>" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-12">
                            ...
                            <select id="id_akun_debit" name="id_akun_debit"  class="form-control" required>
                                <option disabled selected style="display: none;" value="">Pilih Akun</option>
                                <?php if(count($piutang) > 0 ) { ?>
                                    <?php foreach ($piutang as $row):?>
                                        <option value="<?= $row->id_akun?>"><?= $row->id_akun?> - <?= $row->nama_akun?></option>
                                    <?php endforeach; ?>
                                <?php } else { ?>
                                    <option value="" disabled selected>Akun Piutang Tidak Tersedia, Silahkan Tambahkan Terlebih Dahulu</option>
                                <?php } ?>
                            </select>
                        </div>
