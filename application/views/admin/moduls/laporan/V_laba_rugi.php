...
?>
<?php $title = "<i class='fas fa-dollar-sign'></i>&nbsp;Laba Rugi"; ?>
...
<div class="row">
    <div class="col-md-12" style="margin-bottom: 10px">
        <form action="<?= base_url('C_laporan/laba_rugi_filter_tanggal') ?>" method="POST">
    ...
    <div class="col-md-12">
        <table class="table table-striped table-bordered table-hover" id="tableView" cellspacing="0" width="100">
            ...
            <tbody>
                <!-- PEMASUKAN -->
                <tr>
                    <td colspan="3"><b>Pemasukan</b></td>
                </tr>
                <?php
                if (count($pemasukan) > 0) {
                    foreach ($pemasukan as $key => $value) {
                        $saldoPemasukan = $value->debitTotal - $value->kreditTotal; ?>
                        <tr>
                            <td></td>
                            <td><?= $value->id_akun ?> - <?= $value->nama_akun ?></td>
                            <td>Rp. <?= number_format(abs($saldoPemasukan), 0, ".", ".") ?></td>
                        </tr>
                    <?php } ?>
                    <?php
                        $total_debit = 0;
                        ...
                        $totalSaldoPemasukan = $total_debit - $total_kredit;
                    ?>
                        <tr>
                            <td></td>
                            <td><b>Jumlah</b></td>
                            <td>Rp. <?php echo number_format(abs($totalSaldoPemasukan), 0, ".", ".") ?></td>
                        </tr>
                <?php } else { ?>
                    <tr>
                ...
                if (count($pengeluaran) > 0) {
                    foreach ($pengeluaran as $key => $value) { 
                        $saldoPengeluaran = $value->debitTotal - $value->kreditTotal; ?>
                        <tr>
                            <td></td>
                            <td><?= $value->id_akun ?> - <?= $value->nama_akun ?></td>
                            <td>Rp. <?= number_format(abs($saldoPengeluaran), 0, ".", ".") ?></td>
                        </tr>
                    <?php } ?>
                    <?php
                        $total_debit = 0;
                        ...
                        $totalSaldoPengeluaran = $total_debit - $total_kredit;
                    ?>
                        <tr>
                            <td></td>
                            <td><b>Jumlah</b></td>
                            <td>Rp. <?php echo number_format(abs($totalSaldoPengeluaran), 0, ".", ".") ?></td>
                        </tr>
                <?php } else { ?>
                    <tr>
                        <td colspan="7" class="text-center">Tidak ada data yang tersedia</td>
                ...
                <?php } ?>
                    ...
                    <?php $laba_rugi = abs($totalSaldoPemasukan) - abs($totalSaldoPengeluaran); ?>
                    <tr>
                        <td></td>
                        <?php if($laba_rugi > 0 ) { ?>
                            <td><b>Laba</b></td>
                        <?php } else { ?>
                            <td><b>Rugi</b></td>
                        <?php } ?>
                        <td>Rp. <?= number_format(abs($laba_rugi), 0, ".", ".") ?></td>
                    </tr>
            </tbody>
        </table>
    </div>
