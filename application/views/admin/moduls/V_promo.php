...
?>
<?php $title = "<i class='fa fa-gift'></i>&nbsp;Promo"; ?>
...
<div class="row">
    <div class="col-md-12">
        <div style="padding-bottom: 10px;">
            <a href="#tambahproduk" role="button" class="btn btn-primary" data-toggle="modal">Tambah Promo</a>
        </div>
                ...
                <div class="modal-content">
                    <form class="form-horizontal" role="form" id="formAddPengalaman" action="<?= base_url(uri_string() . '/add_produk') ?>" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="modal-header">
                            ...
                            <h3 class="smaller lighter blue no-margin">Tambah Promo</h3>
                        </div>
                        <div class="modal-body">
                            <div class="row">
                                ...
                                <div class="col-md-12">
                                    ...
                                    <input type="text" class="form-control" name="judul_promo" placeholder="Judul Promo" required>
                                </div>

                                <div class="col-md-12">
                                    <label>Poster Promo</label>
                                    <input type="file" class="form-control" name="poster_promo" id="input_foto5" required>
                                </div>

                                <div class="col-md-12">
                                    <label>Deskripsi Promo</label>
                                    <textarea class="form-control" name="deskripsi_promo" id="input_deskripsi" placeholder="Deskripsi Promo"></textarea>
                                </div>
            ...
            <tbody>
                ...
                if (count($result) > 0) {
                    ...
                    foreach ($result as $key => $value) { ?>
                        <tr>
                            <td><?= $start++ ?></td>
                            <td><?= $value->judul_promo ?></td>
                            <td class="text-center">
                                <?php if ($value->poster_promo) { ?>
                                    <img src="<?= base_url('assets/img/' . $value->poster_promo) ?>" width="100">
                                <?php } ?>
                            </td>
                            <td><?= $value->deskripsi_promo ?></td>
                            <td><?= $value->tanggal_mulai ?></td>
                            <td><?= $value->tanggal_akhir?></td>
                            <td>
                                <button type="button" class="btn btn-primary btn-xs" onclick="edit_produk('<?= $value->id_promo ?>')">
                                    <i class="fa fa-edit"> Edit</i>
                                </button>
                                <button type="button" class="btn btn-danger btn-xs" onclick="delete_produk('<?= $value->id_promo ?>')">
