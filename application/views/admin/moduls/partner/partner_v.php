    ...
    <div id="Kuliner" class="city">
      <div style="padding: 20px 0 25px 0;">
            <a href="#tambahprodukkuliner" role="button" class="btn btn-primary" data-toggle="modal">Tambah Partner</a>
        </div>

        <div id="tambahprodukkuliner" class="modal fade" tabindex="-1">
            <div class="modal-dialog modal-lg">
                <div class="modal-content">
                    <form class="form-horizontal" role="form" id="formAddKuliner" action="<?= base_url('C_partner/add_produk') ?>" method="POST" enctype="multipart/form-data">
                        <input type="reset" class="hidden">
                        <div class="modal-header">
                            <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
                            <h3 class="smaller lighter blue no-margin">Tambah Partner</h3>
                        </div>
                        <div class="modal-body">
                            <div class="row">
                                ...
                                <div class="col-md-12">
                                    ...
                                    <textarea class="form-control" name="deskripsi_partner" id="input_deskripsi_kuliner" placeholder="Deskripsi Partner"></textarea>
                                </div>
                                ...
                                <?php foreach ($id_jenis as $key => $value) { ?>
                                <input type="hidden" class="form-control" name="jenis_partner" value="<?= $value->id_jenis_partner ?>">
                                <?php } ?>
                            </div>
                        </div>
                ...
                <?php
                    ...
                    foreach ($result as $key => $value) { ?>
                        ...
                        <tr>
                            ...
                            <td><?= $value->deskripsi_partner ?></td>
                            <td class="text-center">
                                <?php if ($value->logo_partner) { ?>
                                    <img src="<?= base_url('assets/img/' . $value->logo_partner) ?>" width="100">
                                <?php } ?>
                            </td>
                            <td><?= $value->website_partner ?></td>
                            <td><?= $value->email_partner ?></td>
                            <td><?= $value->kontak_partner?></td>
                            <td>
                                <button type="button" class="btn btn-primary btn-xs" onclick="edit_produk('<?= $value->id_partner ?>')">
                                    <i class="fa fa-edit"> Edit</i>
                                </button>
                                <button type="button" class="btn btn-danger btn-xs" onclick="delete_produk('<?= $value->id_partner ?>')">
    ...
    <script>
        ...
        tinyMCE.init({
            mode: "exact",
            elements: "input_deskripsi_kuliner",
            theme: "advanced",
        ...
        $('#formAddKuliner').submit(function(e) {
        e.preventDefault();
        tinyMCE.triggerSave();
    ...
    $("a[href='#tambahprodukkuliner']").click(function() {
