    <div id="IT" class="city" style="display: none;">
      <div style="padding: 20px 0 25px 0;">
            <a href="#tambahprodukit" role="button" class="btn btn-primary" data-toggle="modal">Tambah Partner IT</a>
        </div>

        <div id="tambahprodukit" class="modal fade" tabindex="-1">
            <div class="modal-dialog modal-lg">
                <div class="modal-content">
                    <form class="form-horizontal" role="form" id="formAddIt" action="<?= base_url(uri_string() . '/add_produk') ?>" method="POST" enctype="multipart/form-data">
                        <input type="reset" class="hidden">
                        <div class="modal-header">
                            <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
                            <h3 class="smaller lighter blue no-margin">Tambah Partner IT</h3>
                        </div>
                        <div class="modal-body">
                            <div class="row">
                                ...
                                <div class="col-md-12">
                                    ...
                                    <textarea class="form-control" name="deskripsi_partner" id="input_deskripsi_it" placeholder="Deskripsi Partner"></textarea>
                                </div>
                                ...
                                <input type="hidden" class="form-control" name="jenis_partner" value="2">
            ...
            <tbody>
                <?php
                    ...
                    foreach ($result as $key => $row) { 
                        if ($row->jenis_partner == 2) {
                            ?>
                        
                        <tr>
                            <td><?= $mulai++ ?></td>
                            <td><?= $row->nama_partner ?></td>
                            <td><?= $row->deskripsi_partner ?></td>
                            <td class="text-center">
                                <?php if ($row->logo_partner) { ?>
                                    <img src="<?= base_url('assets/img/' . $row->logo_partner) ?>" width="100">
                                <?php } ?>
                            </td>
                            <td><?= $row->website_partner ?></td>
                            <td><?= $row->email_partner ?></td>
                            <td><?= $row->kontak_partner?></td>
                            <td>
                                <button type="button" class="btn btn-primary btn-xs" onclick="edit_produk('<?= $row->id_partner ?>')">
                                    <i class="fa fa-edit"> Edit</i>
                                </button>
                                <button type="button" class="btn btn-danger btn-xs" onclick="delete_produk('<?= $row->id_partner ?>')">
    ...
    $("a[href='#tambahprodukit']").click(function() {
