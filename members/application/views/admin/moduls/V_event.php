...
?>
<?php $title = "<i class='fa fa-list-ul'></i>&nbsp;Event"; ?>
...
<div class="row">
    <div class="col-md-12">
        <div style="padding-bottom: 10px;">
            <a href="#tambahproduk" role="button" class="btn btn-primary" data-toggle="modal">Tambah Event</a>
        </div>
                ...
                <div class="modal-content">
                    <form class="form-horizontal" role="form" id="formAddPengalaman" action="<?= base_url(uri_string() . '/add_produk') ?>" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="modal-header">
                            ...
                            <h3 class="smaller lighter blue no-margin">Tambah Event</h3>
                        </div>
                        <div class="modal-body">
                            <div class="row">
                                ...
                                <div class="col-md-12">
                                    ...
                                    <input type="text" class="form-control" name="judul_event" placeholder="Judul Event" required>
                                </div>

                                <div class="col-md-12">
                                    <label>Poster Event</label>
                                    <input type="file" class="form-control" name="poster_event" id="input_foto5" required>
                                </div>

                                <div class="col-md-12">
                                    <label>Deskripsi Event</label>
                                    <textarea class="form-control" name="deskripsi_event" id="input_deskripsi" placeholder="Deskripsi Event"></textarea>
                                </div>

                                <div class="col-md-12">
                                    <label>Tanggal Event</label>
                                    <input type="date" class="form-control" name="tanggal_event" placeholder="Tanggal Event" required>
                                </div>

                                <div class="col-md-12">
                                    <label>Jam Event</label>
                                    <input type="time" class="form-control" name="jam_event" placeholder="Jam Event" required>
                                </div>
            ...
            <tbody>
                ...
                if (count($result) > 0) {
                    ...
                    foreach ($result as $key => $value) { ?>
                        <tr>
                            <td><?= $start++ ?></td>
                            <td><?= $value->judul_event ?></td>
                            <td class="text-center">
                                <?php if ($value->poster_event) { ?>
                                    <img src="<?= base_url('assets/img/' . $value->poster_event) ?>" width="100">
                                <?php } ?>
                            </td>
                            <td><?= $value->deskripsi_event ?></td>
                            <td><?= $value->tanggal_event ?></td>
                            <td><?= $value->jam_event ?></td>
                            <td>
                                <button type="button" class="btn btn-primary btn-xs" onclick="edit_produk('<?= $value->id_event ?>')">
                                    <i class="fa fa-edit"> Edit</i>
                                </button>
                                <button type="button" class="btn btn-danger btn-xs" onclick="delete_produk('<?= $value->id_event?>')">
