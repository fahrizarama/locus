...
class C_komentar extends CI_Controller
{
    ...
    public function komentar_members($id_artikel) {
        $slider = $this->Beranda_m->getAll2();
		$data['slider'] = $slider;

        $data['result'] = $this->komentar->komentarfind('', $id_artikel);
        $data['members'] = $this->member_m->getAllKategori()->result();

        $this->load->view('parts/header', $data);
        if($this->session->userdata('id_kategori') == '1') {
            $this->load->view('member/personal/komentar', $data);
        } elseif($this->session->userdata('id_kategori') == '2') {
            $this->load->view('member/komunitas/komentar', $data);
        } elseif($this->session->userdata('id_kategori') == '3') {
            $this->load->view('member/perusahaan/komentar', $data);
        } else {
            $this->load->view('member/pendidikan/komentar', $data);
        }
